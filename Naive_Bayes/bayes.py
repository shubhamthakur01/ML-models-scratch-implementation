...
def load_docs(docs_dirname:str) -> Sequence[Sequence]:
    ...
    docs = [words(get_text(file)) for file in allfiles if '.txt' in file]
    return docs
...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    V = defaultintdict() 
    V['__unknown__'] = 0 
    dist_words = {word for lst in neg + pos for word in lst}
    sorted_words = sorted(dist_words)
...
def vectorize_docs(docs:Sequence, V:dict) -> np.ndarray:
    ...
    """
    lst = [vectorize(V, doc) for doc in docs]
    return np.array(lst)
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        """
        self.p_pos = np.sum(y)/len(y)
        self.p_neg = 1- self.p_pos
        V = X.shape[1]
        # For positive 
        pos = X[np.where(y == 1)]
        self.prob_w_pos=(np.sum(pos, axis= 0)+1)/(np.sum(pos) + V)
        # For negatives
        neg = X[np.where(y == 0)]
        self.prob_w_neg=(np.sum(neg, axis= 0)+1)/(np.sum(neg) + V)
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        """
        likelihood_neg= np.log(self.p_neg) + np.sum(np.log(self.prob_w_neg)*X, axis =1)
        likelihood_pos = np.log(self.p_pos) + np.sum(np.log(self.prob_w_pos)*X, axis =1)
        predictions = np.zeros(X.shape[0],  dtype=int )
        predictions[np.where(likelihood_neg < likelihood_pos)] = 1
        return predictions
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
    ...
    for train_index, test_index in kf.split(X):
        ...
        acc=len([p for p, y in zip(preds, y_test) if p == y])/len(preds)
